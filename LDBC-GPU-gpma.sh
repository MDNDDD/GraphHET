...
# chmod +x ./LDBC-GPU-gpma.sh

data_dir="${DATA_DIR:-/home/mabojing/data}"
repo_dir="${REPO_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
executable="${EXECUTABLE:-${repo_dir}/build/bin_gpu/Test_GPU_GPMA}"
log_dir="${LOG_DIR:-${repo_dir}/logs}"
input_file="${INPUT_FILE:-input.txt}"
result_file="${RESULT_FILE:-result-gpu-gpma.csv}"
gpu_id="${GPU_ID:-0}"
...
do
    ...
    do
        ...
        CUDA_VISIBLE_DEVICES="${gpu_id}" "${executable}" < "${input_file}" > "${log_dir}/${filename}-GPU-gpma.output"
